...
export class ListComponent implements OnInit, OnDestroy {
  ...
  readonly title: string = TITLES.BREAKS;
  ...
  // Define observables with takeUntil to automatically unsubscribe when component is destroyed
  breaks$: Observable<Inassist[]> = this.store.select(InassistState.getItems).pipe(takeUntil(this.destroy$));
  ...
  ngOnDestroy(): void {
    ...
    this.store.dispatch(new InassistAction.ClearAll);
    this.store.dispatch(new InassistAction.ClearItemSelection);
  }
  ...
  private initializeComponent(): void {
    ...
    if (companies.length > 1) {
      this.store.dispatch(new InassistAction.GetAll());
      this.config = {
        search: true,
        company: true,
        customer: true,
      ...
      this.store.dispatch(new InassistAction.GetAll(this.companyId));
      this.config = {
        search: true,
        customer: true,
        unit: true,
